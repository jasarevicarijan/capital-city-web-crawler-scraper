...
export class CapitalCityScraper {
  ...
  async scrapeCity(url: string) {
    ...
    const country =
      $(".infobox th:contains(Country) + td").first().text().trim() ||
      undefined;

    const areaRows = $(".mergedtoprow th:contains(Area)")
      .parent()
      .nextUntil(".mergedtoprow")
      .addBack();
    const areaValues = areaRows.find("td").filter(
      (i, el) =>
        !!$(el)
          .text()
          .trim()
    ...
    const populationRows = $(".mergedtoprow th:contains(Population)")
      .parent()
      .nextUntil(".mergedtoprow")
      .addBack();
    const populationValues = populationRows.find("td").filter(
      (i, el) =>
        !!$(el)
          .text()
          .trim()
    ...
    const population =
      parseFloat(populationText.replace(/,/g, "")) || undefined;

    const flagPageLink = $(".infobox a.image + div:contains(Flag)")
      .prev()
      .attr("href")!;
